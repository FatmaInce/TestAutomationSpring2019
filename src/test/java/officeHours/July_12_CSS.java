...
public class July_12_CSS {
    ...
    css:
        ...
        benefits:faster than xpath, easier to read.
                 it's also used more often like xpath

        write xpath with attributes:
           formula://tag name[@Attribute='value']
           if not unique: //tag name[@Attribute='value' and @Attribute2='value']

        Values Can not start with numbers in css
        write css with attribute:
            formula:tag name[Attribute=Value]

            // double quote or single qoute is not necessary
            if not unique :   tag name[Attribute=Value][Attribute2=Value]

        write css with parent and child tagnames:
                    parentTagname childtagname[Attribute=Value]

        write css with ID: if the id contains multiple digits, it's most likely to be dynamic
                in css # represents your id
                    formula:  tag name#IDValue

        write css with id and attribute: we use it if the id is not unique
                formula:  tag name#idvalue[Attribute=Value]

        write css with class:
                        . sign is for class
                      formula:  tag name.ClassValue

        write css with class and attribute:   if the class is not unique
               formula:  tag name.ClassValue[Attribute=Value]

        write css with start with condition:
                    ^ sign is for "starting with:
                    formula:  tag name[Attribute^=StartValue]

        write css with ends with condition:
                $ sign is for ends with:
                formula: tag name[Attribute$=endingValue]

        write css with contains condition:
                * sign is for contains condition
                formula: tag name[Attribute*=someValue]
     */
